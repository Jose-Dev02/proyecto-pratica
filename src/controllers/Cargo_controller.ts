...
const leerCargosById = async (req: Request, res: Response) => {
  try {
    const response = await Cargo.findOne({ _id: req.params.id }).select({
      __v: 0,
    });
...
const actualizarCargo = async (req: Request, res: Response) => {
  ...
  try {
    const old_response = await Cargo.findOne({ _id: req.params.id }).select({
      __v: 0,
    });
    ...
    const new_response = await Cargo.findByIdAndUpdate(
      { _id: req.params.id },
      req.body,
      { new: true }
    );
